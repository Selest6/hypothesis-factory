...
- Источники (sources) — только из контекста, формат {file, sheet?, row?, page?, fragment?}:
  ...
  • Литература PDF (текстовый слой): file + page
...
КРИТИЧЕСКИ ВАЖНО:
- Каждая гипотеза — конкретная комбинация: класс крупности / минерал / потери из Excel + техническое вмешательство из литературы, OCR-схем или графа.
...
- Примеры формулировок от организаторов показывают только СТИЛЬ и уровень конкретности; содержание бери из Excel, литературы и OCR.
{SOURCES_RULES}
...
Контекст из базы знаний (литература, OCR-схемы PNG, OCR-страницы PDF, отчёты):
{retrieved_context}
...
- sources: массив объектов {{file, sheet?, row?, page?, fragment?}} — Excel, PDF, OCR-схемы (*.png), OCR-страницы PDF (file+page); для интернета file = URL
...
- data_anchor: отсылка к файлу/строке/странице/фрагменту из контекста (Excel, PDF, OCR-схема PNG или OCR-страница PDF)
...
def _context_user_block(
    *,
    ...
    return (
        ...
        f"Контекст из базы знаний (литература, OCR-схемы PNG, OCR-страницы PDF, отчёты):\n{retrieved_context}\n\n"
